...
var (
	minGoSwissMapsVersion = semver.New(1, 24, 0, "", "")

	goWithSwissMaps = probe.PackageConstraints{
		Package: "std",
		Constraints: func() *semver.Constraints {
			c, err := semver.NewConstraint(">= " + minGoSwissMapsVersion.String())
			if err != nil {
				panic(err)
			}
			return c
...
func (c swissMapsUsedConst) InjectOption(td *process.TargetDetails) (inject.Option, error) {
	isUsingGoSwissMaps := td.GoVersion.GreaterThanEqual(minGoSwissMapsVersion)
