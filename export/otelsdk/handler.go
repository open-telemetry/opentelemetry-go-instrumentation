...
// NewHandler returns a new configured [export.Handler] that uses the
// OpenTelemetry SDK (go.opentelemetry.io/otel/sdk) to process and export
// telemetry generated by auto-instrumentation.
func NewHandler(ctx context.Context, options ...Option) (*export.Handler, error) {
	c, err := newConfig(ctx, options)
	if err != nil {
		return nil, err
	}
	return newHandler(c), nil
}

func newHandler(c config) *export.Handler {
	return &export.Handler{TraceHandler: newTraceHandler(c)}
}
...
var _ export.TraceHandler = (*TraceHandler)(nil)
